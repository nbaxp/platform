...
var dataImage = function (size, text) {
  ...
  assert(
    ...
    'Invalid size, expected INTxINT, e.g. 300x400',
  )
  {
      ...
      __assign(
        __assign(
          __assign(
            __assign(
              __assign(
                __assign(__assign({ extend: extendFunc }, basic), date$1),
                image$1,
              ),
              color$1,
            ),
...
var parse = function (name) {
  ...
  var range =
    parameters && parameters[3] && parameters[3].match(constant.RE_RANGE)
  ...
  var count = range
    ...
    : undefined
  var decimal =
    parameters && parameters[4] && parameters[4].match(constant.RE_RANGE)
...
var handler$1 = {
  ...
  gen: function (template, name, context) {
    ...
    context = {
      ...
      templateRoot:
        context.templateRoot || context.templateCurrentContext || template,
    }
    var rule = parse(name)
  ...
  object: function (options) {
    ...
    if (options.rule.min != undefined) {
      ...
      for (var key in options.template) {
        var target =
          typeof options.template[key] === 'function' ? fnKeys : keys$1
        target.push(key)
      }
  ...
  boolean: function (options) {
    ...
    var result = options.rule.parameters
      ? random.bool(
          Number(options.rule.min),
          Number(options.rule.max),
          options.template,
        )
      : options.template
    return result
  ...
  placeholder: function (placeholder, obj, templateContext, options) {
    ...
    for (var i = 0; i < params.length; i++) {
      ...
      if (constant.RE_PLACEHOLDER.test(params[i])) {
        params[i] = handler$1.placeholder(
          params[i],
          obj,
          templateContext,
          options,
  ...
  getValueByKeyPath: function (key, options) {
    ...
    if (key.charAt(0) === '/') {
      absolutePathParts = [options.context.path[0]].concat(
        handler$1.normalizePath(keyPathParts),
      )
    } else {
      // 相对路径
      if (keyPathParts.length > 1) {
        ...
        absolutePathParts = handler$1.normalizePath(
    ...
    try {
      ...
      if (
        ...
        // 先计算被引用的属性值
        templateCurrentContext[key] = handler$1.gen(
...
var Diff = {
  diff: function (schema, data, name) {
    ...
    if (
      Diff.name(schema, data, name, result) &&
      Diff.type(schema, data, name, result)
    ) {
...
var Assert = {
  ...
  match: function (type, path, actual, expected, result, message) {
    ...
    result.push(
      Assert.createDiffResult(type, path, actual, expected, message, 'matches'),
    )
    return false
  },
...
function mock(rurl, rtype, template) {
  assert(
    arguments.length,
    'The mock function needs to pass at least one parameter!',
  )
  // Mock.mock(template)
  if (arguments.length === 1) {
